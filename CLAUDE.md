...
- **Only files ending in `.symlink` get installed.** The Rakefile globs `**/*.symlink`
  ...
  are not linked directly into `$HOME`; instead they are `source`d from an installed
  symlink file. Concretely: `zsh/zshrc.symlink` sources `~/.dotfiles/shell/common` by
  absolute path assuming the repo lives at `~/.dotfiles`.
- Directories are organized by tool: `zsh/`, `shell/`, `git/`.
...
- `shell/common` holds PATH, `$EDITOR`, aliases for git, the `reload` alias, and
  `zoxide`/`mise` init.
